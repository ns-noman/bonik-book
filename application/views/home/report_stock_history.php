    ...
    <section class="content">
      <div class="card">
        <div class="card-header">
          <form class="form-horizontal" action="<?php echo base_url() . "stock-history-search-report"; ?>" method="post" enctype="multipart/form-data"  role="form" >
            <div class="form-group row">
              <label for="category" class="col-sm-1 col-form-label">Date <span class="text-red">*</span></label>
              <div class="col-sm-2">
                  <input type="text" class="form-control datepicker" name="history_date" id="history_date" autocomplete="off" placeholder="yyyy-mm-dd" required>
              </div>
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            <thead>
                                <tr style="background-color: #dfdfdf; text-align: center;">
                                    ...
                                    <!-- <th>Purchase Qty</th>
                                    <th>Sales Qty</th> -->
                                    <th width="150">Opening Qty</th>
                                    <!-- <th width="150">Closing Qty</th> -->
                                    <th>Purchase Rate</th>
                                    <th>Stock Value</th>
                                </tr>
                                <?php 
                                ...
                                foreach($products as $product){
                                    $stock_history = $this->inventory_model->get_stock_history($product->product_id, $date);
                                    $closing_day = date('Y-m-d', strtotime("+1 day", strtotime($date)));
                                    //$closing_history = $this->inventory_model->get_stock_history($product->product_id, $closing_day);
                                    if ($stock_history) {
                                    $stock_value += $stock_history->stock_qty*$stock_history->stock_unit_price;
                                ?>
                                <tr>
                                    <td><?php echo $x++;?></td>  
                                    <td><?php echo $product->product_name;?></td>
                                    <td><?php echo $product->product_category_name;?></td>
                                    <!-- <td><?php //echo $purchase_qty." ".$product->unit_name;?></td> -->
                                    <!-- <td><?php //echo $sales_qty." ".$product->unit_name;?></td> -->
                                    <td><?php echo $stock_history->stock_qty." ".$product->unit_name;?></td>
                                   <!--  <td><?php //if($closing_history){echo $closing_history->stock_qty." ".$product->unit_name;}?></td> -->
                                    <td align="right"><?php echo number_format($product->product_unit_price, 2, '.', ',');?></td>
                                    <td align="right"><?php echo number_format($stock_history->stock_qty*$stock_history->stock_unit_price, 2, '.', ',');?></td>
                                </tr>
                            <?php }} ?>
                                <tr style="font-weight:bold; background-color: #dfdfdf;">
                                    <td align="right" colspan="5">Total:</td>  
                                    <td align="right"><?php echo number_format($stock_value, 2, '.', ',');?></td>  
                                </tr>
                            </thead>
                            <tbody>
