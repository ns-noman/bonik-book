<!-- Content Header (Page header) -->
    ...
    padding: 2px;
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1>Sales Requisition#: <?php echo $sales_info->sales_req_no; ?></h1>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="card">
        ...
        if($this->session->userdata('user_type') == "Client" or in_array('New Requisition', $privilege_set)){
         ?>
          ...
          <?php } 
          if($sales_info->sales_req_status == 1){
             if($this->session->userdata('user_type') == "Client" or in_array('Approved Requisition', $privilege_set)){
           ?>
          <a href="<?php echo base_url() . "approve-sales-requisition/" . $sales_info->sales_req_id; ?>" class="btn btn-warning btn-xs">Approve</a>
      <?php } 
      if($this->session->userdata('user_type') == "Client" or in_array('Sales Requisition Cancel', $privilege_set)){
      ?>
          <a href="<?php echo base_url() . "cancel-sales-requisition/" . $sales_info->sales_req_id; ?>" class="btn btn-danger btn-xs">Cancel</a>
      <?php }} ?>
                ...
                <table class="table table-bordered">
                    ...
                    foreach($sales_item as $item){
                        ...
                        <tr>
                            ...
                            <td align="center"><?php echo $item->req_item_quantity." ".$item->unit_name;?></td>
                            <td align="right"><?php echo number_format((float)$item->req_item_amount/$item->req_item_quantity, 2, '.', '');?></td>
                            <td align="right"><?php echo $item->req_item_vat_per; ?></td>
                            <td align="right"><?php echo number_format((float)$item->req_item_amount+$vat, 2, '.', '');?></td>
                        </tr>
