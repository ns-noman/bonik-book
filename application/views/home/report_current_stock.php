                        ...
                        <table class="table table-bordered table-striped table-hover">
                            <thead>
                                ...
                                foreach($products as $product){
                                    //$purchase_item = $this->inventory_model->get_purchase_qty($product->product_id);
                                   //$sales_item = $this->inventory_model->get_sales_qty($product->product_id);
                                    //$purchase_qty = $purchase_item->purchase_item_quantity-$purchase_item->purchase_return_item_quantity;
                                    //$sales_qty = $sales_item->sales_itemS_quantity-$sales_item->sales_return_item_quantity;
                                    if($product->product_stock>0){
                                    $stock_value += $product->product_stock*$product->product_unit_price;
                                    $opening_qty = $this->inventory_model->get_stock_history($product->product_id, date('Y-m-d'));
                                ...
                                <tr>
                                    ...
                                    <td><?php if($opening_qty){echo $opening_qty->stock_qty." ".$product->unit_name;}else{echo "0 ".$product->unit_name;}?></td>
